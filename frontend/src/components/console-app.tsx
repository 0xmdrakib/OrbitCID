...
export function ConsoleApp() {
  ...
  async function saveActivity(action: "file.uploaded" | "pin.created" | "pin.removed", subject: string, metadata: Record<string, string | number | boolean | null> = {}) {
  ...
  return <main className="console-main">
    <section className="console-hero"><div><p className="eyebrow">YOUR CONTROL SPACE</p><h1>Good to see you,<br/><em>{session.user.name?.split(" ")[0] || "operator"}.</em></h1><p>Neon keeps this account isolated. Your content remains on the IPFS backend you own.</p></div><div className="identity-card">{session.user.image ? <img src={session.user.image} alt="" referrerPolicy="no-referrer"/> : <span>{session.user.name?.[0] || "U"}</span>}<div><small>VERIFIED GOOGLE IDENTITY</small><strong>{session.user.name}</strong><p>{session.user.email}</p></div><Icon name="check"/></div></section>
    ...
    {!!connections.some((connection) => connection.state === "active") && <section className="workspace-section"><div className="workspace-title"><div><p className="eyebrow">DIRECT IPFS WORKSPACE</p><h2>Work with your node</h2><p>File bytes stream from this browser to your backend. Vercel receives only the resulting activity record.</p></div><label>Active backend<select value={activeConnectionId} onChange={(event) => { setActiveConnectionId(event.target.value); setPins([]); }}>{connections.filter((connection) => connection.state === "active").map((connection) => <option value={connection.id} key={connection.id}>{connection.name}</option>)}</select></label></div><div className="console-grid">
