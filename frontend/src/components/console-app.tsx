...
export function ConsoleApp() {
  ...
  return <main className="console-main">
    ...
    {!!connections.some((connection) => connection.state === "active") && <section className="workspace-section"><div className="workspace-title"><div><p className="eyebrow">DIRECT IPFS WORKSPACE</p><h2>Work with your node</h2><p>File bytes stream from this browser to your backend. The hosted control service receives only the resulting activity record.</p></div><label>Active backend<select value={activeConnectionId} onChange={(event) => { setActiveConnectionId(event.target.value); setPins([]); setBackupStatus(null); }}>{connections.filter((connection) => connection.state === "active").map((connection) => <option value={connection.id} key={connection.id}>{connection.name}</option>)}</select></label></div><div className="console-grid">
      <form className="console-panel upload-panel" onSubmit={upload}><span className="workspace-icon"><BrandMark size={38} tone="light"/></span><p className="eyebrow">STREAMING UPLOAD</p><h3>Add to Kubo</h3><p>CIDv1, raw leaves, SHA-256 and 1 MiB chunks are enforced by the backend.</p><label className="file-drop"><input type="file" onChange={(event) => setFile(event.target.files?.[0] || null)} required/><strong>{file?.name || "Choose a file"}</strong><small>{file ? `${file.size.toLocaleString()} bytes` : "The file streams directly to your backend."}</small></label><button className="ink-button wide" disabled={busy || !file}>Upload and pin <Icon name="arrow"/></button></form>
