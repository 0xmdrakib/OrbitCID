...
export function ConsoleApp() {
  ...
  async function configureBackup(event: FormEvent) {
    ...
    try {
      ...
      setNotice("R2 backup configured on this backend. The secret was not stored by Vercel or Neon."); await refresh();
  ...
  return <main className="console-main">
    ...
    {activeConnection && <section className="console-panel backup-panel"><div className="panel-head"><div><p className="eyebrow">OPTIONAL OFFSITE BACKUP</p><h2>Cloudflare R2</h2><p>Keep this disabled if you do not need an offsite copy. Credentials travel directly to your paired backend and are encrypted there; Vercel and Neon never store them.</p></div><button className="refresh-button" onClick={() => void loadBackupStatus()} disabled={busy}>Check status</button></div>
