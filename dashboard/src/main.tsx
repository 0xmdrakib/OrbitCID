...
function AccessRequired(){return <div className="gate"><div className="gate-watermark" aria-hidden="true">ACCESS REQUIRED</div><div className="gate-orbit" aria-hidden="true"><span/><span/><span/></div><section className="gate-card"><div className="gate-brand"><div className="brand-mark large"><Icon name="shield" size={28}/></div><div><strong>OrbitCID</strong><small>Owner console</small></div></div><p className="eyebrow">FAIL-CLOSED SECURITY</p><h1>Google sign-in required.</h1><p className="gate-copy">This request did not contain a verified Cloudflare Access identity. Open the protected API hostname and sign in with an allowed account. OrbitCID has no local or development login bypass.</p>{API_ORIGIN&&<a className="primary access-link" href={apiUrl("/")}>Open protected API</a>}</section></div>}
